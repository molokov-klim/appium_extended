    ...
    def _wait_for(self,
        ...
        if image is not None:
            ...
            for i in image:
                ...
                while not self.helper._is_image_on_the_screen(image=i, threshold=threshold) and time.time() - start_time < timeout_method:
                    time.sleep(sleep)
                if not self.helper._is_image_on_the_screen(image=i, threshold=threshold):
                    return False
    ...
    def _wait_for_not(self,
        ...
        if image is not None:
            ...
            while time.time() - start_time < timeout_method:
                ...
                for i in image:
                    # Check if the image is on the screen within the timeout period
                    if self.helper._is_image_on_the_screen(image=i, threshold=threshold):
                        images_present = True
