...
class HelpersDecorators:
    ...
    def wait_until_window_change(self, poll_frequency: float = 0.1):
        """
        ...
        def func_decorator(func):
            ...
            def wrapper(object_instance, *args, **kwargs):
                """
                ...
                func_result = func(object_instance, *args,
                                   **kwargs)  # Вызов декорированной функции и сохранение результата
                ...
                while time.time() - start_time < timeout_window:  # Продолжаем до достижения тайм-аута
                    ...
                    while time.time() - window_not_changing_period_start_time < window_not_changing_period:
                        ...
                        image_bytes = object_instance.driver.get_screenshot_as_png()
                        ...
                        new_image_bytes = object_instance.driver.get_screenshot_as_png()
    ...
    def wait_for_window_change(self, poll_frequency: float = 0.5):
        """
        ...
        def func_decorator(func):
            ...
            def wrapper(object_instance, *args, **kwargs):
                ...
                image_bytes = object_instance.driver.get_screenshot_as_png()  # Сделать снимок экрана окна и получить данные изображения в виде байтов
                ...
                box = (
                    50, 50, 400,
                    400)  # Определить координаты прямоугольной области для обрезки (лево, верх, право, низ)
                ...
                for _ in range(tries):  # Выполнить попытки обнаружения на основе заданного числа попыток
                    ...
                    func_result = func(object_instance, *args,
                                       **kwargs)  # Выполнить декорированную функцию и сохранить результат
                    ...
                    while time.time() - start_time < timeout_window:  # Проверить, находится ли прошедшее время в пределах заданного окна времени ожидания
                        ...
                        new_image_bytes = object_instance.driver.get_screenshot_as_png()  # Сделать новый снимок экрана окна и получить данные изображения в виде байтов
                        ...
                        new_image = new_image.crop(
                            box)  # Обрезать новое изображение на основе заданных координат области
    ...
    def step_info(self, my_str):
        ...
        def func_decorator(func):
            ...
            def wrapper(object_instance, *args, **kwargs):
                ...
                screenshot = object_instance.driver.get_screenshot_as_png()
                ...
                video_filename = f'screenrecord_{timestamp}.mp4'  # Имя файла видеозаписи с временной меткой
                object_instance.driver.start_recording_screen()

                try:
                    # Выполняем исходную функцию
                    result = func(object_instance, *args, **kwargs)
                    ...
                    allure.attach(base64.b64decode(object_instance.driver.stop_recording_screen()),
                                  name=video_filename,
